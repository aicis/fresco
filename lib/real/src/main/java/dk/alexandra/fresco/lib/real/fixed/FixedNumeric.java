...
public class FixedNumeric implements RealNumeric {
  ...
  private static DRes<SInt> scale(ProtocolBuilderNumeric scope, DRes<SInt> n, int scale) {
    if (scale >= 0) {
      ...
      n = new DefaultAdvancedNumeric(scope).truncate(n, -scale);
    }
    return n;
  }
  ...
  public DRes<SReal> div(DRes<SReal> a, DRes<SReal> b) {
    return builder.seq(seq -> {
      ...
      DRes<SInt> result = new DefaultAdvancedNumeric(seq).div(scaledA, fixedB.getSInt());
  ...
  public DRes<SReal> div(DRes<SReal> a, BigDecimal b) {
    return builder.seq(seq -> {
      ...
      DRes<SInt> scaledA = scale(seq, fixedA.getSInt(), seq.getBasicNumericContext().getPrecision());
      BigInteger unscaledB = unscaled(b, seq.getBasicNumericContext().getPrecision());
      DRes<SInt> result = new DefaultAdvancedNumeric(seq).div(scaledA, unscaledB);
      return new SFixed(result);
    });
  }
  ...
  public DRes<SInt> leq(DRes<SReal> a, DRes<SReal> b) {
    return builder.seq(seq -> {
      SFixed fixedA = (SFixed) a.out();
      SFixed fixedB = (SFixed) b.out();
      return new DefaultComparison(seq).compareLEQ(fixedA.getSInt(), fixedB.getSInt());
    });
  }
}
