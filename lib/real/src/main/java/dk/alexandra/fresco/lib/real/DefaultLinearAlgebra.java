...
public abstract class DefaultLinearAlgebra implements RealLinearAlgebra {
    ...
    return builder.par(par -> {
      Matrix<DRes<SReal>> matrix =
          new Matrix<>(a.getHeight(), a.getWidth(), i -> new ArrayList<>(a.getRow(i).stream()
              .map(e -> new FixedNumeric(par).input(e, inputParty)).collect(Collectors.toList())));
      return () -> matrix;
    });
  }
  ...
  public DRes<ArrayList<DRes<SReal>>> input(ArrayList<BigDecimal> a, int inputParty) {
    return builder.par(par -> {
      ArrayList<DRes<SReal>> matrix = a.stream().map(e -> new FixedNumeric(par).input(e, inputParty))
  ...
  public DRes<Matrix<DRes<BigDecimal>>> openMatrix(DRes<Matrix<DRes<SReal>>> a) {
    return builder.par(par -> {
      Matrix<DRes<BigDecimal>> matrix = new Matrix<>(a.out().getHeight(), a.out().getWidth(),
          i -> new ArrayList<>(a.out().getRow(i).stream().map(e -> new FixedNumeric(par).open(e))
  ...
  public DRes<ArrayList<DRes<BigDecimal>>> openArrayList(DRes<ArrayList<DRes<SReal>>> a) {
    return builder.par(par -> {
      ArrayList<DRes<BigDecimal>> vector = a.out().stream().map(e -> new FixedNumeric(par).open(e))
          .collect(Collectors.toCollection(ArrayList::new));
      return () -> vector;
    });
  }
  ...
  public DRes<Matrix<DRes<SReal>>> add(DRes<Matrix<DRes<SReal>>> a, DRes<Matrix<DRes<SReal>>> b) {
    return builder.par(par -> {
      return entrywiseBinaryOperator(par, a.out(), b.out(),
          (builder, x) -> new FixedNumeric(builder).add(x.getFirst(), x.getSecond()));
    });
  }
  ...
  public DRes<Matrix<DRes<SReal>>> sub(DRes<Matrix<DRes<SReal>>> a, DRes<Matrix<DRes<SReal>>> b) {
    return builder.par(par -> {
      return entrywiseBinaryOperator(par, a.out(), b.out(),
          (builder, x) -> new FixedNumeric(builder).sub(x.getFirst(), x.getSecond()));
    });
  }
  ...
  public DRes<Matrix<DRes<SReal>>> mult(DRes<Matrix<DRes<SReal>>> a, Matrix<BigDecimal> b) {
    return builder.seq(seq -> {
      return mult(seq, a.out(), b, (scope, x) -> new AdvancedFixedNumeric(scope)
  ...
  public DRes<Matrix<DRes<SReal>>> mult(Matrix<BigDecimal> a, DRes<Matrix<DRes<SReal>>> b) {
    return builder.seq(seq -> {
      return mult(seq, a, b.out(), (builder, x) -> new AdvancedFixedNumeric(builder)
  ...
  public DRes<Matrix<DRes<SReal>>> mult(DRes<Matrix<DRes<SReal>>> a, DRes<Matrix<DRes<SReal>>> b) {
    return builder.seq(seq -> {
      return mult(seq, a.out(), b.out(),
          (builder, x) -> new AdvancedFixedNumeric(builder).innerProduct(x.getFirst(), x.getSecond()));
    });
  }
  ...
  public DRes<Matrix<DRes<SReal>>> scale(BigDecimal s, DRes<Matrix<DRes<SReal>>> a) {
    return builder.par(par -> {
      return scale(par, s, a.out(),
          (builder, x) -> new FixedNumeric(builder).mult(x.getFirst(), x.getSecond()));
    });
  }
  ...
  public DRes<Matrix<DRes<SReal>>> scale(DRes<SReal> s, Matrix<BigDecimal> a) {
    return builder.par(par -> {
      return scale(par, s, a,
          (builder, x) -> new FixedNumeric(builder).mult(x.getSecond(), x.getFirst()));
    });
  }
  ...
  public DRes<ArrayList<DRes<SReal>>> vectorMult(DRes<Matrix<DRes<SReal>>> a, ArrayList<BigDecimal> v) {
    return builder.par(par -> {
      return vectorMult(par, a.out(), v, (builder, x) -> new AdvancedFixedNumeric(builder)
          .innerProductWithPublicPart(x.getSecond(), x.getFirst()));
    });
  }
  ...
  public DRes<ArrayList<DRes<SReal>>> vectorMult(Matrix<BigDecimal> a, DRes<ArrayList<DRes<SReal>>> v) {
    return builder.par(par -> {
      return vectorMult(par, a, v.out(), (builder, x) -> new AdvancedFixedNumeric(builder)
          .innerProductWithPublicPart(x.getFirst(), x.getSecond()));
    });
  }
