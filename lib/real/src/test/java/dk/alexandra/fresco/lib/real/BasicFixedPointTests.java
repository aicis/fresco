      ...
      return new TestThread<ResourcePoolT, ProtocolBuilderNumeric>() {
        ...
        public void test() {
          Application<List<BigDecimal>, ProtocolBuilderNumeric> app = producer -> {
            RealNumeric realNumeric = new FixedNumeric(producer);
            List<DRes<SReal>> inputs = value.stream().map(x -> realNumeric.input(x, 1))
                .collect(Collectors.toList());
            List<DRes<BigDecimal>> opened =
                inputs.stream().map(realNumeric::open).collect(Collectors.toList());
        ...
        public void test() {
          Application<List<BigDecimal>, ProtocolBuilderNumeric> app = producer -> {
            ...
            List<DRes<SReal>> inputs =
                sints.stream().map(realNumeric::fromSInt).collect(Collectors.toList());
            List<DRes<BigDecimal>> opened =
                inputs.stream().map(realNumeric::open).collect(Collectors.toList());
      ...
      return new TestThread<ResourcePoolT, ProtocolBuilderNumeric>() {
        ...
        public void test() {
          Application<BigDecimal, ProtocolBuilderNumeric> app = producer -> {
            RealNumeric realNumeric = new FixedNumeric(producer);
            DRes<SReal> input = realNumeric.input(value, 1);
            return realNumeric.open(input, 1);
          };
      ...
      return new TestThread<ResourcePoolT, ProtocolBuilderNumeric>() {
        ...
        public void test() {
          Application<List<BigDecimal>, ProtocolBuilderNumeric> app = producer -> {
            RealNumeric realNumeric = new FixedNumeric(producer);
            List<DRes<SReal>> inputs =
                value.stream().map(realNumeric::known).collect(Collectors.toList());
            List<DRes<BigDecimal>> opened =
                inputs.stream().map(realNumeric::open).collect(Collectors.toList());
            return () -> opened.stream().map(DRes::out).collect(Collectors.toList());
          };
          ...
          for (int i = 0; i < output.size(); i++) {
            RealTestUtils.assertEqual(value.get(i), output.get(i), DEFAULT_PRECISION + 1);
          }
        }
      };
    }
        ...
        public void test() {
          Application<List<BigDecimal>, ProtocolBuilderNumeric> app = producer -> {
            ...
            List<DRes<SReal>> closed1 =
                openInputs.stream().map(realNumeric::known).collect(Collectors.toList());

            List<DRes<SReal>> result = new ArrayList<>();
            for (DRes<SReal> inputX : closed1) {
              result.add(
                  realNumeric.add(openInputs2.get(closed1.indexOf(inputX)), inputX));
            }

            List<DRes<BigDecimal>> opened =
                result.stream().map(realNumeric::open).collect(Collectors.toList());
            return () -> opened.stream().map(DRes::out).collect(Collectors.toList());
          };
          List<BigDecimal> output = runApplication(app);
          ...
          for (BigDecimal openOutput : output) {
            ...
            RealTestUtils.assertEqual(BigDecimal.valueOf(a + b), openOutput, DEFAULT_PRECISION);
          }
        }
      };
    }
      ...
      return new TestThread<ResourcePoolT, ProtocolBuilderNumeric>() {
        ...
        public void test() {
          Application<List<BigDecimal>, ProtocolBuilderNumeric> app = producer -> {
            ...
            List<DRes<SReal>> closed1 =
                openInputs.stream().map(realNumeric::known).collect(Collectors.toList());
            List<DRes<SReal>> closed2 = openInputs2.stream().map(realNumeric::known)
                .collect(Collectors.toList());

            List<DRes<SReal>> results = new ArrayList<>();
            for (DRes<SReal> inputX : closed1) {
              results.add(realNumeric.sub(inputX, closed2.get(closed1.indexOf(inputX))));
            }

            List<DRes<BigDecimal>> opened =
                results.stream().map(realNumeric::open).collect(Collectors.toList());
            return () -> opened.stream().map(DRes::out).collect(Collectors.toList());
          };
          List<BigDecimal> output = runApplication(app);
            ...
            List<DRes<BigDecimal>> opened = Stream
                .concat(
                    IntStream.range(0, closed.size()).mapToObj(
                        i -> realNumeric.sub(closed.get(i), openInputs2.get(i))),
                    IntStream.range(0, closed.size()).mapToObj(
                        i -> realNumeric.sub(openInputs2.get(i), closed.get(i))))
                .map(realNumeric::open).collect(Collectors.toList());
            return () -> opened.stream().map(DRes::out).collect(Collectors.toList());
          };
          List<BigDecimal> output = runApplication(app);
      ...
      return new TestThread<ResourcePoolT, ProtocolBuilderNumeric>() {
        ...
        public void test() {
          Application<List<BigDecimal>, ProtocolBuilderNumeric> app = producer -> {
            ...
            for (DRes<SReal> inputX : closed1) {
              result.add(
                  realNumeric.mult(openInputs2.get(closed1.indexOf(inputX)), inputX));
            }
      ...
      return new TestThread<ResourcePoolT, ProtocolBuilderNumeric>() {
        ...
        public void test() {
          Application<BigDecimal, ProtocolBuilderNumeric> app = producer -> {
            ...
            DRes<SReal> result = realNumeric.known(BigDecimal.ONE);
            for (DRes<SReal> inputX : closed1) {
              result = realNumeric.mult(result, inputX);
            }

            DRes<BigDecimal> opened = realNumeric.open(result);
            return opened;
          };
          BigDecimal output = runApplication(app);
      ...
      return new TestThread<ResourcePoolT, ProtocolBuilderNumeric>() {
        ...
        public void test() {
          Application<BigDecimal, ProtocolBuilderNumeric> app = producer -> {
            ...
            DRes<SReal> input = realNumeric.input(value, 1);
            DRes<SReal> product = realNumeric.div(input, value2);

            return realNumeric.open(product);
          };
          BigDecimal output = runApplication(app);
      ...
      return new TestThread<ResourcePoolT, ProtocolBuilderNumeric>() {
        ...
        public void test() {
          Application<List<BigDecimal>, ProtocolBuilderNumeric> app = producer -> {
            ...
            List<DRes<SReal>> result = new ArrayList<>();
            for (DRes<SReal> inputX : closed1) {
              result.add(realNumeric.mult(inputX, closed2.get(closed1.indexOf(inputX))));
            }
      ...
      return new TestThread<ResourcePoolT, ProtocolBuilderNumeric>() {
        ...
        public void test() {
          Application<List<BigDecimal>, ProtocolBuilderNumeric> app = producer -> {
            ...
            List<DRes<SReal>> result = new ArrayList<>();
            for (DRes<SReal> inputX : closed1) {
              result.add(realNumeric.add(inputX, closed2.get(closed1.indexOf(inputX))));
            }
      ...
      return new TestThread<ResourcePoolT, ProtocolBuilderNumeric>() {
        ...
        public void test() {
          Application<List<BigDecimal>, ProtocolBuilderNumeric> app = producer -> {
            ...
            List<DRes<SReal>> result = new ArrayList<>();
            for (DRes<SReal> inputX : closed1) {
              result.add(realNumeric.div(inputX, closed2.get(closed1.indexOf(inputX))));
            }
            List<DRes<BigDecimal>> opened =
                result.stream().map(realNumeric::open).collect(Collectors.toList());
            return () -> opened.stream().map(DRes::out).collect(Collectors.toList());
      ...
      return new TestThread<ResourcePoolT, ProtocolBuilderNumeric>() {
        ...
        public void test() {
          Application<List<BigInteger>, ProtocolBuilderNumeric> app = producer -> {
            ...
            for (DRes<SReal> inputX : closed1) {
              result.add(realNumeric.leq(inputX, closed2.get(closed1.indexOf(inputX))));
            }
