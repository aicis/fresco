...
public class TestOrderMatchingDemo extends AbstractSpdz2kTest<Spdz2kResourcePool<CompUInt64>> {
  public static class TestOrderMatchingSimple<ResourcePoolT extends ResourcePool>
    ...
    public static final List<OrderMatch> expected = Arrays.asList(
    ...
    public TestThread<ResourcePoolT, ProtocolBuilderNumeric> next() {
      ...
      return new TestThread<ResourcePoolT, ProtocolBuilderNumeric>() {
        ...
        public void test() {
          ...
          List<List<BigInteger>> processedOrders = runApplication(distDemo);
          for (int i = 0; i < expected.size(); i++) {
            Assert.assertEquals(processedOrders.get(i).get(0).intValueExact(),
                expected.get(i).firstId);
            Assert.assertEquals(processedOrders.get(i).get(1).intValueExact(),
                expected.get(i).secondId);
            Assert.assertEquals(processedOrders.get(i).get(2).intValueExact(),
                expected.get(i).rate);
          }
        }
      };
  ...
  public void testPlainOrderMatching() {
    ...
    List<OrderMatch> res = matching.compute();
