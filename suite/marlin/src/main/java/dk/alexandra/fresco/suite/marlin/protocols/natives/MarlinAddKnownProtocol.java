...
public class MarlinAddKnownProtocol<PlainT extends CompUInt<?, ?, PlainT>>
  ...
  public MarlinAddKnownProtocol(PlainT input, DRes<SInt> right) {
    this.left = input;
    this.right = right;
  }
  ...
  public EvaluationStatus evaluate(int round, MarlinResourcePool<PlainT> resourcePool,
      Network network) {
    out = ((MarlinSInt<PlainT>) right.out()).addConstant(left, resourcePool.getMyId(),
