...
public class SemiHonestOTExtensionReceiver implements
  ...
  private Random random;
	...
	public List<BitSet> receive(List<OTSigma> sigmas, int expectedLength) {
		 ...
		 * the matrix T.
		 */
    List<dk.alexandra.fresco.suite.tinytables.ot.datatypes.OTInput> otInputs = new ArrayList<dk.alexandra.fresco.suite.tinytables.ot.datatypes.OTInput>();
    for (int i = 0; i < securityParameter; i++) {
			BitSet x0 = t.getColumn(i);
			BitSet x1 = BitSetUtils.copy(x0);
			x1.xor(r);
      otInputs.add(new dk.alexandra.fresco.suite.tinytables.ot.datatypes.OTInput(
