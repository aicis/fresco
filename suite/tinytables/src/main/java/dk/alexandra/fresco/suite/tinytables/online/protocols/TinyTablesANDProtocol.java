  ...
  public EvaluationStatus evaluate(int round, ResourcePoolImpl resourcePool, Network network) {
    ...
    if (round == 0) {
      ...
      for (byte[] bytes : buffers) {
        shares.add(TinyTablesElement.getTinyTablesElement(BooleanSerializer.fromBytes(bytes[0])));
      }
      boolean open = TinyTablesElement.open(shares);
      this.out = TinyTablesSBool.getTinyTablesSBool(TinyTablesElement.getTinyTablesElement(open));
