  ...
  public EvaluationStatus evaluate(int round, ResourcePoolImpl resourcePool, Network network) {
    ...
    if (round == 0) {
      if (resourcePool.getMyId() == this.inputter) {
        ...
        TinyTablesElement e = TinyTablesElement.getTinyTablesElement(this.in ^ r.getShare());
        network.sendToAll(new byte[]{BooleanSerializer.toBytes(e.getShare())});
      }
      return EvaluationStatus.HAS_MORE_ROUNDS;
    } else {
      TinyTablesElement share =
          TinyTablesElement.getTinyTablesElement(BooleanSerializer.fromBytes(network.receive(this.inputter)[0]));
      out = TinyTablesSBool.getTinyTablesSBool(share);
