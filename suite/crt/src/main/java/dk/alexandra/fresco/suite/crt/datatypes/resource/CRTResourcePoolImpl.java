...
public class CRTResourcePoolImpl<ResourcePoolA extends NumericResourcePool, ResourcePoolB extends NumericResourcePool> extends ResourcePoolImpl implements
  ...
  public CRTResourcePoolImpl(int myId, int noOfPlayers, CRTDataSupplier dataSupplier, ResourcePoolA resourcePoolLeft, ResourcePoolB resourcePoolRight) {
    ...
    this.definition = new BigIntegerFieldDefinition(
        dataSupplier.getFieldDefinitions().getFirst().getModulus()
            .multiply(dataSupplier.getFieldDefinitions().getSecond().getModulus()));
