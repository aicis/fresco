...
public class DataGeneratorImpl implements DataGenerator {
  ...
  private final Map<Integer, Map<Integer, BlockingQueue<SpdzElement[]>>> expPipesQueue =
      new HashMap<>();
  private final Map<Integer, Map<Integer, Map<Integer, BlockingQueue<SpdzInputMask>>>> inputMaskQueue =
      new HashMap<>();
  ...
  private void init() {
    ...
    for (Type t : Type.values()) {
      ...
      for (int threadId = 0; threadId < noOfVMThreads; threadId++) {
          ...
          case INPUT:
            ...
            for (int myId = 1; myId <= noOfPlayers; myId++) {
              GeneratorThread thread =
                  new GeneratorThread(this, t, amount, noOfPlayers, threadId, alpha, mod, myId);
              thread.start();
              threads.add(thread);
            }
        ...
        GeneratorThread thread =
            new GeneratorThread(this, t, amount, noOfPlayers, threadId, alpha, mod);
        thread.start();
        threads.add(thread);
      }
    }
  ...
  @Override
  public void addInputMasks(int towards, List<SpdzInputMask[]> inpMasks, int thread)
      throws InterruptedException {
    for (int i = 0; i < noOfPlayers; i++) {
