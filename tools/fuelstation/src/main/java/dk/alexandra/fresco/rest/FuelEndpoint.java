...
public class FuelEndpoint {
  ...
  public void init() {
    ...
    System.out.println("FuelEndPoint uses mod size: "+ modulusSize);
  }
  
  /**
   * REST endpoint for fetching a given amount of triples for the wanted
   * party. Note that this is currently insecure as any party can ask for any
   * other party's preprocessed material.
   * 
   * @param amount
   *            The amount of triples to procure
   * @param partyId
   *            The ID of the owning party
   * @param thread
   *            The thread id - used for parallel runs
   * @param response
   *            Autowired by spring- needed for streaming the data
   * @throws InterruptedException
   *             If the thread fetching from the blocking queue gets
   *             interrupted.
   * @throws IOException
   *             If the stream gets cut.
   */
  @RequestMapping(value = "/triples/{amount}/party/{partyId}/thread/{thread}", method = RequestMethod.GET)
  private void generateTriples(@PathVariable int amount, @PathVariable int partyId, @PathVariable int thread,
  ...
  /**
   * Same as {@link #generateTriples(int, int, int, HttpServletResponse)}, but
   * for bits instead.
   */
  @RequestMapping(value = "/bits/{amount}/party/{partyId}/thread/{thread}", method = RequestMethod.GET)
  private void generateBits(@PathVariable int amount, @PathVariable int partyId, @PathVariable int thread,
      HttpServletResponse response) throws InterruptedException, IOException {
  ...
  /**
   * Same as {@link #generateTriples(int, int, int, HttpServletResponse)}, but
   * for exponentiation pipes instead.
   */
  @RequestMapping(value = "/exp/{amount}/party/{partyId}/thread/{thread}", method = RequestMethod.GET)
  private void generateExpPipes(@PathVariable int amount, @PathVariable int partyId, @PathVariable int thread,
      HttpServletResponse response) throws InterruptedException, IOException {
  ...
  /**
   * Same as {@link #generateTriples(int, int, int, HttpServletResponse)}, but
   * for input masks instead
   */
  @RequestMapping(value = "/inputs/{amount}/party/{partyId}/towards/{towardsId}/thread/{thread}", method = RequestMethod.GET)
  private void generateInputMasks(@PathVariable int amount, @PathVariable int partyId, @PathVariable int towardsId,
      @PathVariable int thread, HttpServletResponse response) throws InterruptedException, IOException {
  ...
  /**
   * NB: This is insecure as any party can call this end point and obtain the
   * secret key of any party.
   * 
   * @param partyId
   *            The ID of the party whoose secret key to obtain.
  ...
  /**
   * Calling this method has no effect unless all parties called it. When that
   * happens, the effect is the same as restarting the service. This means
   * that all data in queues are thrown away, and generator threads are
   * restarted.
   * 
   * @param partyId
   *            The ID of the party calling this end point.
   * @return True if the request was accepted - this does not necessarily mean
