...
public class SIntWrapperLinearAlgebra implements LinearAlgebra {
  ...
  public DRes<Matrix<DRes<SFixed>>> mult(DRes<Matrix<DRes<SFixed>>> a, Matrix<BigDecimal> b) {
    return builder.seq(seq -> {
      return mult(seq, a.out(), b,
          (f, x) -> innerProductWithPublicPart(f, x.getSecond(), x.getFirst()));
    });
  }
  ...
  public DRes<Matrix<DRes<SFixed>>> mult(Matrix<BigDecimal> a, DRes<Matrix<DRes<SFixed>>> b) {
    return builder.seq(seq -> {
      return mult(seq, a, b.out(),
          (f, x) -> innerProductWithPublicPart(f, x.getFirst(), x.getSecond()));
    });
  }
  ...
  public DRes<Matrix<DRes<BigDecimal>>> open(DRes<Matrix<DRes<SFixed>>> a) {
    DRes<Matrix<DRes<SInt>>> closedAsInts =
        () -> mapMatrix(a.out(), entry -> ((SFixedSIntWrapper) entry.out()).getSInt());
  ...
  private <A> DRes<Matrix<DRes<SFixed>>> add(ProtocolBuilderNumeric builder, Matrix<A> a,
      Matrix<DRes<SFixed>> b,
      BiFunction<BasicFixedNumeric, Pair<A, DRes<SFixed>>, DRes<SFixed>> add) {
    return builder.par(par -> {
  ...
  private <A, B> DRes<Matrix<DRes<SFixed>>> mult(ProtocolBuilderNumeric builder, Matrix<A> a,
      Matrix<B> b,
      BiFunction<ProtocolBuilderNumeric, Pair<List<A>, List<B>>, DRes<SFixed>> innerProduct) {
    return builder.par(par -> {
  ...
  private <S, A> DRes<Matrix<DRes<SFixed>>> scale(ProtocolBuilderNumeric builder, S s, Matrix<A> a,
    ...
    return builder.par(par -> {
      ...
      for (ArrayList<A> row : a.getRows()) {
        rows.add(new ArrayList<>(row.stream().map(x -> mult.apply(fixed, new Pair<>(s, x)))
  ...
  private DRes<SFixed> innerProductWithPublicPart(ProtocolBuilderNumeric builder,
      List<BigDecimal> a, List<DRes<SFixed>> b) {
    if (a.size() != b.size()) {
    ...
    List<BigInteger> aInt =
        a.stream().map(x -> x.setScale(precision, RoundingMode.DOWN).unscaledValue())
            .collect(Collectors.toList());
    List<DRes<SInt>> bInt =
        b.stream().map(x -> ((SFixedSIntWrapper) x.out()).getSInt()).collect(Collectors.toList());
    DRes<SInt> innerProductInt = builder.advancedNumeric().innerProductWithPublicPart(aInt, bInt);
    DRes<SInt> innerProductUnscaled =
        builder.advancedNumeric().div(innerProductInt, BigInteger.TEN.pow(precision));
  ...
  public DRes<SFixed> innerProduct(ProtocolBuilderNumeric builder, List<DRes<SFixed>> a,
      List<DRes<SFixed>> b) {
    if (a.size() != b.size()) {
      throw new IllegalArgumentException("Vectors must have same size");
    }
    List<DRes<SInt>> aInt =
        a.stream().map(x -> ((SFixedSIntWrapper) x.out()).getSInt()).collect(Collectors.toList());
    List<DRes<SInt>> bInt =
        b.stream().map(x -> ((SFixedSIntWrapper) x.out()).getSInt()).collect(Collectors.toList());
