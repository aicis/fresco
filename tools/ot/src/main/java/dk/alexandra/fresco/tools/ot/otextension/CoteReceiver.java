...
public class CoteReceiver extends CoteShared {
   ...
   * @param network The network object used to communicate with the other party
   */
  ...
  public List<StrictBitVector> extend(StrictBitVector choices) {
    ...
    final List<StrictBitVector> tlistZero = ParallelStreaming.apply(
        prgs,
        parallel -> parallel
    ...
    final List<StrictBitVector> ulist = ParallelStreaming.apply(
        prgs,
        parallel -> parallel
            .limit(resources.getComputationalSecurityParameter())
            .map(p -> p.getSecond())
    ...
    ParallelStreaming.call(
        ulist.parallelStream(),
        parallel -> parallel.forEach(u -> u.xor(choices))
    );
    ParallelStreaming.call(
        IntStream.range(0, resources.getComputationalSecurityParameter()),
        parallel -> parallel.forEach(i -> ulist.get(i).xor(tlistZero.get(i)))
    );
    sendList(ulist);
