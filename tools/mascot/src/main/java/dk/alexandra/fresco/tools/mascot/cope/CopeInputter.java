...
public class CopeInputter {
   ...
   */
  public List<MascotFieldElement> extend(List<MascotFieldElement> inputElements) {
    // use seeds to generate mask pairs
    List<Pair<MascotFieldElement, MascotFieldElement>> maskPairs = generateMaskPairs(inputElements.size());
    // compute t0 - t1 + x for each input x for each mask pair
    List<MascotFieldElement> diffs = helper.computeDiffs(maskPairs, inputElements);
    // send diffs
  ...
  private List<Pair<MascotFieldElement, MascotFieldElement>> generateMaskPairs(int numInputs) {
  ...
  private List<Pair<MascotFieldElement, MascotFieldElement>> generateMaskPairs(Modulus modulus) {
    Stream<Pair<MascotFieldElement, MascotFieldElement>> maskStream =
        IntStream.range(0, leftPrgs.size()).parallel().mapToObj(idx -> {
          ...
          MascotFieldElement t1 = this.rightPrgs.get(idx).getNext(modulus);
