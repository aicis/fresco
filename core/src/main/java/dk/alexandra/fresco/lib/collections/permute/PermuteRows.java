...
public class PermuteRows implements ComputationBuilder<Matrix<Computation<SInt>>> {
  ...
  private PermuteRows(Matrix<Computation<SInt>> values, int[] idxPerm, int permProviderPid,
      boolean isPermProvider, boolean singletonRows, BasicNumericFactory bnf) {
    super();
    this.values = values;
  ...
  // private List<SInt[]> doRound(List<SInt[]> roundInputs, SInt[][] cbits, int numRows, int
  // numCols,
  // int colIdx) {
  ...
  private static final class IterationState implements Computation<IterationState> {
    ...
    private IterationState(int round, Computation<SInt> value) {
      this.round = round;
      this.value = value;
    }
