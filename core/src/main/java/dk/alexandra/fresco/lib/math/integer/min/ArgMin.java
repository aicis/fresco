  ...
  public DRes<Pair<List<DRes<SInt>>, SInt>> buildComputation(
    ...
    if (this.size == 2) {
      ...
      DRes<SInt> secondCompare = numeric
          .subFromOpen(builder.getOIntFactory().one(), firstCompare);
      return () -> new Pair<>(
      ...
      DRes<SInt> tmp = numeric.subFromOpen(builder.getOIntFactory().one(), firstComparison);
      ...
      return builder.seq((seq) -> {
        ...
        DRes<SInt> oneMinusCompare = numeric.subFromOpen(seq.getOIntFactory().one(), compare);
