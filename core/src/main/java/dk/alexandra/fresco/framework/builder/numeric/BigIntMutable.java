...
public class BigIntMutable extends Number {
  ...
  /*** <Constructors> ***/
  /**
   * Creates a BigIntM from the given parameters.
   * The input-array will be used as is and not be copied.
   *
   * @param sign The sign of the number.
   * @param v The magnitude of the number, the first position gives the least significant 32
   *     bits.
   ...
   *
   * @param sign The sign of the number.
   * @param v The magnitude of the number, the first position gives the least significant 8
   *     bits.
  ...
  /**
   * Creates a BigIntM from the given int.
  ...
  /**
   * Creates a BigIntM from the given long.
   * The input-value will be interpreted a signed value.
   *
   * @param val The value of the number.
   * @complexity O(1)
  ...
  /**
   * Creates a BigIntM from the given string.
   *
  ...
  /**
   * Creates a BigIntM from the given char-array.
   *
  ...
  /**
   * Assigns the given number to this BigIntM object.
   *
  ...
  /**
   * Assigns the given BigIntM parameter to this number.
  ...
  /**
   * Assigns the given BigIntM parameter to this number.
   * Assumes no leading zeroes of the input-array, i.e. that v[vlen-1]!=0, except for the case when
   * vlen==1.
   *
   * @param sign The sign of the number.
   * @param v The magnitude of the number.
  ...
  public void assign(final char[] s) {
    ...
    final int alloc =
        len < 10 ? 1 : (int) (len * 3402L >>> 10) + 32 >>> 5; //3402 = bits per digit * 1024
  ...
  /**
   * Assigns the given non-negative number to this BigIntM object.
   *
  ...
  /**
   * {@inheritDoc}
   * Returns this BigIntM as a {@code byte}.
   *
  ...
  /**
   * {@inheritDoc}
   * Returns this BigIntM as a {@code short}.
   *
  ...
  /**
   * {@inheritDoc}
   * Returns this BigIntM as an {@code int}.
   *
  ...
  /**
   * {@inheritDoc}
   * Returns this BigIntM as a {@code long}.
   *
   * @return {@code sign * (this & 0x7FFFFFFFFFFFFFFF)}
   */
  @Override
  public long longValue() {
    return len == 1 ? sign * (dig[0] & mask)
        : sign * ((dig[1] & 0x7FFFFFFFL) << 32 | (dig[0] & mask));
  }

  /**
   * {@inheritDoc}
   * Returns this BigIntM as a {@code float}.
   *
   * @return the most significant 24 bits in the mantissa (the highest order bit obviously being
   *     implicit),
   *     the exponent value which will be consistent for {@code BigIntM}s up to 128 bits (should it
   *     not
   *     fit it'll be calculated modulo 256),
  ...
  /**
   * {@inheritDoc}
   * Returns this BigIntM as a {@code double}.
   *
   * @return the most significant 53 bits in the mantissa (the highest order bit obviously being
   *     implicit),
   *     the exponent value which will be consistent for {@code BigIntM}s up to 1024 bits (should it
   *     not fit it'll be calculated modulo 2048),
   *     and the sign bit set if this number is negative.
   */
  @Override
  ...
  public void uadd(final long a) //Refactor? Similar to usub.
  {
    if (sign < 0) {
      final long ah = a >>> 32, al = a & mask;
      if (len > 2 || len == 2 && ((dig[1] & mask) > ah
          || (dig[1] & mask) == ah && (dig[0] & mask) >= al) || ah == 0 && (dig[0] & mask) >= al) {
        usubMag(a);
        return;
      }
      if (dig.length == 1) {
  ...
  public long udiv(final long div) //Adaption of general div to long.
  {
    ...
    long u2 = 0, u1 = dig[len - 1] >>> 32 - s, u0 =
        (dig[len - 1] << s | dig[len - 2] >>> 32 - s) & mask;
    if (s == 0) {
      u1 = 0;
      u0 = dig[len - 1] & mask;
    }
    for (int j = len - 2; j >= 0; j--) {
      ...
      while (qhat + hbit >= (1L << 32) + hbit
          || qhat * dl + hbit > (rhat << 32) + u0 + hbit) //Unsigned comparison.
      {
        --qhat;
        rhat = rhat + dh;
   ...
   */
  public void add(
      final int add) //Has not amortized O(1) due to the risk of alternating +1 -1 on continuous sequence of 1-set bits.
  {
    if (add < 0) {
      usub(-add);
    } else {
  ...
  /**
   * Subtracts a BigIntM from this number.
   *
  ...
  /**
   * Multiplies this number by the given BigIntM.
  ...
  /**
   * Multiplies this number by the given (suitably small) BigIntM.
  ...
  /**
   * Multiplies this number by the given BigIntM using the Karatsuba algorithm.
   *
   * @param mul The number to multiply with.
  ...
  /**
   * Divides this number by the given BigIntM.
  ...
  /**
   * Divides this number by the given BigIntM and returns the remainder.
   * Division by zero is undefined.
   *
   * @param div The number to divide with.
   ...
   * @complexity O(m * n)
   */
  private void div(final int[] u, final int[] v, final int m, final int n,
      final int[] q) {//Hacker'sDelight's implementation of Knuth's Algorithm D
    ...
    for (j = m - n; j >= 0; j--) //Main loop
    {
      ...
      while (qhat + hbit >= b + hbit || qhat * dl + hbit > b * rhat + (u[j + n - 2] & mask)
          + hbit) //Unsigned comparison.
      {
        qhat = qhat - 1;
        rhat = rhat + dh;
        if (rhat + hbit >= b + hbit) {
  ...
  static BigIntMutable fromBytes(byte[] bytes) {
    ...
    int len = usedBytes / 4;
    if (usedBytes % 4 != 0 || usedBytes == 0) {
      len++;
    }

    int[] dig = new int[len];

    for (int i = bytes.length - 1, bytesInInt = 0, intIndex = 0; i >= byteLen; i--) {
      byte currentByte = bytes[i];
      int nextByteInPlace = (currentByte & 0xff) << (8 * bytesInInt);
      dig[intIndex] = dig[intIndex] | nextByteInPlace;
      bytesInInt++;
      if (bytesInInt % 4 == 0) {
        intIndex++;
        bytesInInt = 0;
