...
public class ByteArrayHelper {
  ...
  /**
   * Helper method which takes a byte array and returns a new array 2 bytes longer, with the length
   * prepended to the array. This means that the length of the byte array cannot be longer than
   * 65.536.
   * 
   * @param b The array to add a size to.
   ...
   * {@link #addSize(byte[])}. It peels off the length bytes and returns the correct number of bytes
   * back. It is assumed that the length will be at the current position of the given ByteBuffer.
   * 
   * @param buffer The container for the byte array.
