 ...
 * @version 0.7
 */
public class BigInt extends Number implements BigIntegerI {
   ...
   *     implicit),
   *     the exponent value which will be consistent for {@code BigInt}s up to 128 bits (should it
   *     not
   ...
   *     implicit),
   *     the exponent value which will be consistent for {@code BigInt}s up to 1024 bits (should it
   ...
   * @complexity O(n ^ 1.585)
   */
  public void karatsuba(final BigInt mul,
  ...
  public void mod(final BigInt mod) {
    rem(mod);
    if (sign < 0) {
      add(mod);
    }
  ...
  public static BigIntegerI fromBytes(byte[] bytes) {
